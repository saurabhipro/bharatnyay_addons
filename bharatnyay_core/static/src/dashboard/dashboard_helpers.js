...
const DASHBOARD_BATCH_FILTER_STORAGE_KEY = "bharatnyay.dashboard.filter_batch";
    ...
    if (typeof window !== "undefined" && window.localStorage) {
        if (state.filter_batch) {
            window.localStorage.setItem(
                DASHBOARD_BATCH_FILTER_STORAGE_KEY,
                String(state.filter_batch)
            );
        } else {
            window.localStorage.removeItem(DASHBOARD_BATCH_FILTER_STORAGE_KEY);
        }
    }
    await reloadFn();
}
...
export async function restoreDashboardBatchFilter(orm, state) {
    ...
    const savedBatch = window.localStorage.getItem(DASHBOARD_BATCH_FILTER_STORAGE_KEY);
    if (!savedBatch) {
        return;
    }
    state.filter_batch = savedBatch;
    await loadDashboardFilterOptions(orm, state);
    _resetInvalidDashboardFilters(state);
    if (!state.filter_batch) {
        window.localStorage.removeItem(DASHBOARD_BATCH_FILTER_STORAGE_KEY);
    }
